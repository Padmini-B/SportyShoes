...
<body align="center">
	...
	<c:forEach items="${orders}" var="order" varStatus="status" step="1" begin="0">
		<div class="col${status.index % 3 + 1}">
			<c:if test="${order.date == param.date && order.category == param.category}">
				<img src="<c:url value="${order.image}"/>" style="width: 50%; height: 50%; border: 1px solid"/>
				<br><b>Brand: ${order.brand}<br>Category: ${order.category}<br>Mrp: ${order.price}<br>Date: ${order.date}</b><br>
			</c:if>
		</div>
	</c:forEach>
</body>
